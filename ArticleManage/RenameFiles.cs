...
namespace ArticleManage
{
    internal class RenameFiles
    {
        ...
        {
            int counter = 51;
            foreach (var ris_name in this.folders.input_ris_files.filesNames)
            {
                String new_name = ris_name.Replace("MKZ","");
                new_name = "Data" + counter;
                Console.WriteLine($"name: {ris_name}   == >   new: {new_name}");           
                String old_path = this.folders.input_ris_files.folderPath + ris_name;
        ...
        void showTime()
        {
            foreach (var name in folders.input_ris_files.filesNames)
            {
                Console.WriteLine(name);
            }

            //foreach (var path in folders.input_ris_files.filesPaths)
            //{
